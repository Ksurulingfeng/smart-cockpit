...
| 0x11 | ECU 复位 | 先回复 ACK，100ms 后调用 NVIC_SystemReset() |

为实现 UDS 接收，新增了一个 CAN 滤波器组（FilterBank=1），精确匹配 0x7E0，与原有的节点控制帧滤波器（FilterBank=0）并行工作。S3 超时检查集成在 vCanTask 主循环中（每 20ms 执行一次），超时后自动切回默认会话。
